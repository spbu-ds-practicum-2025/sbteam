...
## Нефункциональные требования
  ...
  4. Доступность > 99.9%.
  5. Отказоустойчивость: cистема продолжает работать при сбое одного из узлов.
  ...
  8. Гарантия уникальности сокращённого URL.
...
### Сценарий: Переход по истёкшей ссылке

1. Пользователь переходит по Short URL с истёкшим TTL.
2. Пользователь видит сообщение об ошибке/истечении срока действия.

### Сценарий: Получение метаданных

1. Пользователь отправляет Short URL.
2. Система возвращает метаданные (Long URL, время создания и время истечения).

### Сценарий: Отправка некорректного Long URL

1. Пользователь отправляет в систему Long URL, который не является корректным URL-адресом (например, строка без схемы, с запрещённой схемой или с некорректным форматом).
2. Система возвращает пользователю ошибку валидации и не создаёт Short URL.
...
sequenceDiagram
    ...
    R-->>GW: longUrl
    GW-->>C: HTTP 302 Location: longUrl
```
### Сценарий 3: Переход по ссылке с истекшим TTL
...
### Сценарий 4: Получение метаданных по Short URL

1. Пользователь отправляет HTTP-запрос `GET /api/shortUrls/{shortCode}/meta` в API Gateway.
2. API Gateway по gRPC передаёт запрос в сервис Read API (метод `GetShortUrlMeta`).
3. Read API обращается к Slave ShortURL DB и читает запись по `shortCode` (поля `longUrl`, `createdAt`, `expiresAt` и, при наличии, дополнительные метаданные).
4. Если запись найдена, Read API формирует ответ с метаданными (longUrl, время создания, время истечения) и возвращает его в API Gateway.
5. API Gateway отвечает клиенту `HTTP 200 OK` с JSON-телом, содержащим метаданные.
6. Если запись не найдена или ссылка истекла, Read API возвращает соответствующую ошибку (например, `404 Not Found` или `410 Gone`), а API Gateway проксирует её клиенту.
...
sequenceDiagram
    ...
    C->>GW: GET /api/shortUrls/{shortCode}/meta
    GW->>R: gRPC GetShortUrlMeta(shortCode)
    R->>SDB: SELECT longUrl, createdAt, expiresAt WHERE shortCode = {shortCode}
    alt запись найдена
        SDB-->>R: longUrl, createdAt, expiresAt
        R-->>GW: metadata { longUrl, createdAt, expiresAt }
        GW-->>C: HTTP 200 OK + JSON с метаданными
    else запись не найдена или истекла
        SDB-->>R: not found / expired
        R-->>GW: error (not found / expired)
        GW-->>C: HTTP 404 Not Found или 410 Gone
    end
```

### Сценарий 5: Отправка некорректного Long URL

1. Пользователь отправляет HTTP-запрос `POST /api/shortUrls` в API Gateway с Long URL, который не является корректным URL-адресом с точки зрения бизнес-логики (например, `"not-a-valid-url"` или строка без схемы), но при этом тело запроса:
   - является валидным JSON,
   - содержит поле `longUrl` строкового типа,
   - при необходимости содержит поле `ttl` допустимого типа/диапазона.
2. API Gateway выполняет **базовую техническую валидацию**:
   - проверяет, что запрос имеет корректный `Content-Type` (например, `application/json`);
   - проверяет, что тело — валидный JSON;
   - проверяет наличие обязательных полей (`longUrl`, при необходимости `ttl`) и их типов.
3. Так как структура запроса корректна, API Gateway **не выполняет доменную проверку URL**, а по gRPC передаёт запрос в сервис Write API (метод `CreateShortUrl(longUrl, ttl)`).
4. Write API выполняет **доменную валидацию**:
   - проверяет, что `longUrl` непустой;
   - пытается распарсить `longUrl` как корректный абсолютный URL (например, через `Uri.TryCreate(..., UriKind.Absolute, out _)`);
   - при необходимости проверяет допустимость схемы (`http`, `https` и т. д.).
5. Write API определяет, что Long URL некорректен и **не обращается** к сервису генерации коротких кодов (`ShortCodeGeneratorService`), то есть метод `GenerateShortCodeAsync` не вызывается.
6. Write API формирует ошибку бизнес-валидации (например, с кодом `INVALID_LONG_URL`) и возвращает её в API Gateway.
7. API Gateway отвечает клиенту `HTTP 400 Bad Request` с JSON-телом, описывающим бизнес-ошибку.
8. В master DB, репликах и кэше **не появляются** новые записи, короткий URL не создаётся.
...
sequenceDiagram
    ...
    C->>GW: POST /api/shortUrls { longUrl: "not-a-valid-url", ttl }
    GW->>GW: Базовая валидация (Content-Type, JSON, наличие полей)
    alt Структура запроса некорректна
        GW-->>C: HTTP 400 Bad Request (ошибка формата/схемы)
    else Структура запроса корректна
        GW->>W: gRPC CreateShortUrl(longUrl, ttl)
        W->>W: Доменная валидация longUrl (Uri.TryCreate, схема)
        alt URL некорректен
            W-->>GW: error INVALID_LONG_URL
            GW-->>C: HTTP 400 Bad Request + JSON с бизнес-ошибкой
        else URL корректен
            W->>W: дальнейшая логика (вызов генератора, запись в БД)
            W-->>GW: shortCode / shortUrl
...
**Включает:**
- Реализацию API Gateway (HTTP -> gRPC, роутинг, валидация)
...
2. Реализация API Gateway (роутинг, валидация запросов)
...
- Реализацию rate limiting на уровне API Gateway
---

### **План разработки:**
1. Реализация rate limiting в API Gateway
2. Реализация Analytics Service (подписка на Kafka, запись событий в analytics DB)
3. Добавление аналитических API (количество переходов, горячие ссылки)
4. Добавление health-check, retry-политик
5. Расширенное логирование + метрики (Prometheus/Grafana)
---

### **План тестирования:**
- Тесты rate limiting:
  - превышение лимита запросов для одного пользователя/IP;
  - параллельные запросы от разных пользователей;
  - сброс лимита после окончания окна (запросы снова принимаются);
